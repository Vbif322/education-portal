...
- [ ] **Доступ к видео и прогрессу по праву на урок, а не по факту логина** — `src/app/api/videos/route.ts:11`, `src/app/api/lessons/[lessonId]/progress/route.ts:11-98`. Сейчас любой авторизованный пользователь стримит любой платный ролик по имени файла и читает/пишет прогресс произвольного `lessonId`. Сделать единую функцию `canAccessLesson` (lessonAccess + courseAccess + подписка + public-статус) и вызывать её в обоих роутах; видео-роут должен принимать `lessonId` и брать имя файла из БД, а не сырое `?name=` от клиента.
- [ ] **Открыть публичную страницу курса без логина** — `src/middleware.ts:6`. `publicRoutes = ["/login", "/"]` не включает `/courses/[id]`, хотя страница написана под анонимов. Все карточки курса с лендинга уводят гостя на `/login` — воронка продаж сломана. Заменить точное сравнение на префиксную проверку.
