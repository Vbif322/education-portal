...
- [ ] **Экран «прогресс моей команды»** — на основе существующих `usersToLessons`, `courseAccess`/`lessonAccess` и `src/app/lib/dal/analytics.ts`, отфильтрованных по новой связи «покупатель → подчинённые». (`usersToCourses` больше нет: модель «зачисления» удалена, попадание в «Мои курсы» считает `isOwnCourse` в `course.dal.ts`.)
