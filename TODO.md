...
- [ ] **Форма обратной связи вместо статичного текста.** `src/app/components/dialogs/contact-dialog.tsx` по-прежнему только показывает телефон/почту (теперь кликабельные) и ссылку на `/business`, без полей ввода. Готовый образец для переделки — `src/app/components/lead-form/LeadForm.tsx` + `src/app/actions/lead.ts`.
