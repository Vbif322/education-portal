...
- [x] **Этап 2 — беспарольный вход (magic link) + восстановление пароля + регистрация по ссылке.** Сделаны подпункты 1–5: `sendMail()` с явным получателем (`src/app/lib/email.ts`), таблица `prod.auth_tokens` с колонкой `purpose` и хэшем токена (`src/db/schema/authTokens.ts`, миграция `drizzle/0003_easy_thundra.sql`), выпуск/погашение токенов (`src/app/lib/auth-tokens.ts`), экшены `requestPasswordReset`/`resetPassword`/`requestMagicLink` (`src/app/actions/auth-recovery.ts`), роуты `/api/auth/reset` и `/api/auth/magic`, страницы `/forgot-password` и `/reset-password`, вход по ссылке на `/login` (открыт по умолчанию). Незнакомый адрес не упирается в тупик: на него уходит ссылка на подтверждение, и аккаунт создаётся уже при переходе по ней — до этого в системе нет ничего, кроме строки токена, поэтому занять чужой адрес нельзя. Осталось:
  - [ ] **Доставляемость (был пункт 6, стал срочнее):** выделенный домен отправки, SPF/DKIM/DMARC. Раньше весь поток шёл на один ящик `EMAIL_TO`, теперь письма уходят на произвольные домены пользователей; обработки bounce'ов нет вовсе.
  - [ ] **На будущее: passkeys (WebAuthn)** — целевой стандарт, но тяжелее (управление устройствами, фолбэки).
- [ ] **Закрыть enumeration в регистрации** — `src/app/actions/auth.ts` по-прежнему отвечает «Пользователь с таким email уже существует», и это последнее место, где наличие аккаунта проверяется снаружи: форма входа по ссылке теперь нейтральна в обе стороны (незнакомый адрес получает ссылку на регистрацию, знакомый — на вход, ответ на экране одинаковый). Инфраструктура для закрытия уже есть: `purpose: "signup"` в `auth_tokens` умеет держать адрес без пользователя, а `/api/auth/magic` умеет создавать аккаунт по подтверждению. Останется научить `signup` отвечать нейтрально и слать письмо либо «подтвердите адрес», либо «у вас уже есть аккаунт».

- [ ] **Аккаунты без пароля.** Регистрация по ссылке создаёт пользователя с `password = NULL`: он заходит по ссылке, пока не задаст пароль через «Забыли пароль?». Отдельного экрана «задать пароль» в кабинете нет, и сам пользователь о такой возможности узнаёт только со страницы входа. Сочетается с пунктом про смену пароля из кабинета ниже — делать их стоит вместе.
- [ ] **Смена пароля из кабинета** — экрана нет (`src/app/(lk)/dashboard/profile/page.tsx`), поэтому единственный способ сменить пароль сейчас — письмо через `/forgot-password`. Отсюда же требование, чтобы `/forgot-password` оставался доступен залогиненным (см. комментарий в `src/middleware.ts`).
- [ ] **Регистрозависимый поиск по email** — `eq(users.email, email)` в `signin`/`signup`/`auth-recovery.ts`: зарегистрировавшийся как `A@b.ru` не войдёт как `a@b.ru`. Чинится не точечно: нужен уникальный индекс по `lower(email)` плюс миграция данных с разбором уже существующих дублей.
...
- [ ] **Единый баррель схемы**: удалить `src/db/schema.ts`, оставить `src/db/schema/index.ts` — `src/db/db.ts:2`. (Каждая новая таблица добавляется в оба файла — так сделан и `authTokens`.) Сейчас runtime резолвит неполный файл: `db.query.*` не знает про `userVisits` и все audit-таблицы, а миграции генерируются из полного.
...
- [ ] **Явная проверка `Origin` на мутационных API-роутах (CSRF)** — `/api/lessons/lesson`, `/api/lessons/[lessonId]/progress`. `sameSite: lax` и встроенная защита Server Actions в основном покрывают, но кастомные роуты полагаются только на cookie. Заодно: logout через GET `/api/auth/clear-session` можно принудить `<img>`-тегом — перевести на POST. Туда же `/api/auth/magic`: это мутирующий GET той же формы, риск — login CSRF (жертву насильно логинят в аккаунт атакующего), плюс почтовые сканеры сжигают токен префетчем. Лечится одинаково — промежуточной страницей с кнопкой «Войти».
...
- [ ] **Базовые тесты критичных путей** — тестов и скрипта `test` нет вообще. Минимум: аутентификация, контроль доступа (`canAccessLesson`), upsert прогресса, `consumeAuthToken` (конкурентное погашение одного токена) и контракт нейтрального ответа в `auth-recovery.ts` (Vitest + тестовая БД). Желательно сделать вместе с P0-исправлениями, чтобы зафиксировать их поведение.
