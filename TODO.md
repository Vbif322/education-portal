...
- [ ] **Разделить сценарии B2C и B2B с первого экрана.** Сейчас единый поток для всех — `src/app/page.tsx`. Копирайт про «сотрудников»/компании смешан с частным сценарием обучения (`page.tsx:281,287` — фиче-карточки), отдельного пути для корпоративного заказчика нет вообще.
- [ ] **Карточка курса должна раскрывать содержание, формат и результат**, а не одно предложение — `src/app/components/course-card/CourseCard.tsx:47-111`, поле `description` (`CourseCard.tsx:52`). В схеме `src/db/schema/course.ts:12-23` нет полей под формат/уровень/длительность — понадобится расширение модели.
- [ ] **Результат курса — через навыки и рабочие задачи**, а не абстрактное «повышение эффективности». Опереться на существующий блок «Приобретаемые навыки» (`(public)/courses/[id]/ui.tsx`) и переписать секцию «Почему выбирают наш курс» (`page.tsx:263-291`) в терминах «что участник сможет делать после курса».
- [ ] **Понятный путь от регистрации до результата.** Убрать мёртвый CTA «Выбрать курс» → `/dashboard` → редирект на login без объяснения (`page.tsx:82`, дублирует пункт P1 выше про мёртвые элементы); спроектировать явную последовательность шагов воронки.
- [ ] **Доработать оформление бесплатного/пробного урока.** Сейчас концепции «free/preview» нет вовсе — ни в схеме, ни в UI; единственный путь для неоплатившего — статичный `ContactDialog` (`(public)/courses/[id]/ui.tsx:62-76`). Спроектировать preview-флоу, отталкиваясь от существующего признака `status === "public"` урока, который уже участвует в `canAccessLesson` (`src/app/lib/dal/lesson.dal.ts:29-95`).
- [ ] **FAQ-секция на лендинге** — отсутствует полностью.
- [ ] **Форма обратной связи вместо статичного текста.** `src/app/components/dialogs/contact-dialog.tsx` сейчас только показывает телефон/почту без полей ввода и отправки.
