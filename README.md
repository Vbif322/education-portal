...
| `APP_URL` | нет² | Адрес сайта для ссылок в письмах восстановления доступа; читается только на сервере, меняется рестартом |
| `NEXT_PUBLIC_SITE_URL` | нет² | Публичный адрес сайта для `og:image` и canonical; фолбэк для `APP_URL` |
...
| `EMAIL_TO` | нет¹ | Куда приходят заявки с `/business` (на транзакционные письма не влияет) |
...
пишется в лог сервера. Восстановление пароля и вход по ссылке при этом тоже не
работают — обе формы честно сообщают, что отправка не настроена, и дают
контакты. Остальное приложение работает как обычно.

² В production ссылку в письме строить не из чего, если не задана ни одна из
двух: формы восстановления доступа отвечают «отправка не настроена» и письмо не
уходит. Базу из заголовка `Host` мы намеренно не берём — иначе подделанный
`Host` даёт письмо от нас со ссылкой на сервер атакующего и захват аккаунта.
В разработке фолбэк — `http://localhost:3000`.

### Доставляемость

Транзакционные письма (сброс пароля, вход по ссылке) устроены иначе, чем
заявки: заявки уходят на один известный ящик `EMAIL_TO`, владелец которого
может внести отправителя в белый список, а транзакционные — на произвольные
домены пользователей. Там начинает работать то, чего раньше не было:
грейлистинг незнакомого IP, доля жалоб «спам», отказы на несуществующие
адреса, репутация домена у каждого получателя отдельно. Обработки bounce'ов
нет вовсе — письмо, отвергнутое после `250 OK`, пропадает молча. Практический
минимум перед боевым включением — выделенный домен отправки с SPF, DKIM и
DMARC.
